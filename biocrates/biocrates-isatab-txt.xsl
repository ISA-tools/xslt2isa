...
<!--xsl stylesheet prototype for Biocrates Targed Metabolomics solution XML documents
Author: Philippe Rocca-Serra, University of Oxford (philippe.rocca-serra@oerc.ox.ac.uk) -->
...
<xsl:key name="filelookupid" match="FILE" use="@filename"/>
 ...
 <xsl:text>#Biocrates data document:</xsl:text>  <xsl:value-of select="@identifier"/>
 <xsl:text>
 </xsl:text>
...
<xsl:text></xsl:text>
   ...
   <xsl:choose>
    <xsl:when test="child::EXPERIMENT_REF/@refname">
     ...
     <xsl:text>library construction</xsl:text>   
<xsl:value-of select="key('exptlookupid',child::EXPERIMENT_REF/@refname)/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_STRATEGY/."/>
<xsl:value-of select="key('exptlookupid',child::EXPERIMENT_REF/@refname)/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_SOURCE/."/>
<xsl:value-of select="key('exptlookupid',child::EXPERIMENT_REF/@refname)/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_SELECTION/."/>
<xsl:value-of select="key('exptlookupid',child::EXPERIMENT_REF/@refname)/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_LAYOUT/SINGLE"/>
<xsl:if test="key('exptlookupid',child::EXPERIMENT_REF/@refname)/DESIGN/LIBRARY_DESCRIPTOR/TARGETED_LOCI/LOCUS">
 ...
 <xsl:text>)</xsl:text> 
     ...
     <xsl:if test="contains(key('exptlookupid',child::EXPERIMENT_REF/@refname)/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_CONSTRUCTION_PROTOCOL/.,'pcr_cond: ')">
      <xsl:value-of select="substring-after(key('exptlookupid',child::EXPERIMENT_REF/@refname)/DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_CONSTRUCTION_PROTOCOL/.,'pcr_cond: ')"/>
     </xsl:if>       
     <xsl:value-of select="child::EXPERIMENT_REF/@refname"/>    
    </xsl:when>
    <xsl:otherwise>
...
<xsl:value-of select="@instrument_model"/>   
    </xsl:when>
    <xsl:otherwise>
     <xsl:text></xsl:text>
    </xsl:otherwise>
...
<xsl:value-of select="@run_center"/>    
    </xsl:when>
    <xsl:otherwise>
     <xsl:text></xsl:text>
    </xsl:otherwise>
 ...
 <xsl:text></xsl:text>
   ...
   <xsl:choose>
    <xsl:when test="child::DATA_BLOCK/FILES/FILE/@filename">
     <xsl:value-of select="child::DATA_BLOCK/FILES/FILE/@filename"/>     
     <xsl:value-of select="key('filelookupid',child::DATA_BLOCK/FILES/FILE/@filename)/@checksum_method"/>    
     <xsl:value-of select="key('filelookupid',child::DATA_BLOCK/FILES/FILE/@filename)/@checksum"/> 
    </xsl:when>
    <xsl:otherwise>
     <xsl:text></xsl:text>	
    </xsl:otherwise>
